...
donor_ranking_raw <- safe_plot(
  show_donor_ranking(donor_name = params$name, 
                     year = if (is.null(p_year)) NULL else max(p_year), 
                     top_n_display = 10, 
                     ctr_name = p_ctr, 
                     verbose = FALSE))
...
donor_geographic_diversification_raw <- safe_plot(
...
donor_funding_volatility_raw <- safe_plot(
  show_donor_funding_volatility(donor_name = params$name,
                                year = p_year, 
                                ctr_name = p_ctr,
                                time_unit = params$volatility_time_unit, 
                                forecast_horizon = params$volatility_forecast_h,
                                show_forecast_ci = isTRUE(params$volatility_show_ci), 
                                verbose = FALSE))
...
stories <- c(
  ...
  "Diversification" = generate_plot_story(donor_geographic_diversification_raw, 
                                  provider = params$ai_provider, model = params$ai_model,
                                  max_tokens_short_desc = params$max_tokens_short_desc,
                                  max_tokens_long_desc= params$max_tokens_long_desc),
...
donor_geographic_diversification <- normalize_plot(
  donor_geographic_diversification_raw +
    ggplot2::labs(subtitle = stories["Diversification.short_desc"]))

donor_geographic_diversification
```
...
donor_transaction_desc <- normalize_plot(donor_transaction_desc_raw +
                          ggplot2::labs(subtitle = stories["Interest.short_desc"]))
donor_transaction_desc
```
