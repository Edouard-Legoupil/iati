...
#'
#' @description This function plots the earmarking of funding for a specific implementing partner.
#'
...
#'
#' @export
plot_implementing_partner_earmarking <- function(partner_name,
                                  year = NULL,
                                  by = "global",
                                  programme_lab = NULL,
  ...
  plot_data <- switch(
    ...
    "date" = {
      df |>
        dplyr::group_by(year.x, earmarking_name) |>
    ...
    "country" = {
      df |>
        dplyr::group_by(ctr_name.x, earmarking_name) |>
        dplyr::summarise(total_funding = sum(transaction_value_USD, na.rm = TRUE)) |>
        dplyr::filter(!is.na(ctr_name.x), !is.na(earmarking_name))
    },
    "sector" = {
       df |>
  ...
  p <- ggplot2::ggplot(plot_data, ggplot2::aes(y = total_funding, fill = earmarking_name)) +
    ...
    ggplot2::labs(
      title = paste("Earmarking for Implementing Partner:", partner_name),
